...
public class GuiSettings extends GuiScreen {
    ...
    public void initGui() {
        ...
        this.buttonList.add(
                aNextButton = new GuiButton(-150, width / 2 + 75, height / 2 + 52, 30, 20, ">")); // Static next button
        ...
        for (OreInfo ore : OresSearch.searchList) {
            if (buttons.get(ore.oreName) != null) {
                ...
                int id = Integer.parseInt(Integer.toString(ore.id)
                        + Integer.toString(ore.meta)); // Unique button id. int( str(id) + str(meta) )
                ...
                pageIndex.add(new GuiPage(
                        ...
                        new GuiButton(
                                ...
                                ore.oreName + ": "
                                        + (ore.draw
                                                ? "On"
                                                : "Off")))); // create new button and set the text to Name: On||Off
                buttons.put(
                        ore.oreName,
                        new OreButtons(ore.oreName, id, ore)); // Add this new button to the buttons hashmap.
        ...
        this.buttonList.add(new GuiButton(
                ...
                100,
                20,
                "Print OreDict")); // Static button for search distance.
                ...
                for (String name : OreDictionary.getOreNames()) // Print the ore dictionary.
                {
                    ...
                    for (ItemStack stack : oreStack) {
                        if (stack == oreStack.get(oreStack.size() - 1)) {
                            idMetaCsv.append(String.format(
                                    "%d:%d )", Item.getIdFromItem(stack.getItem()), stack.getItemDamage()));
                        } else {
                            idMetaCsv.append(String.format(
                                    "%d:%d, ", Item.getIdFromItem(stack.getItem()), stack.getItemDamage()));
                        }
    ...
    protected void keyTyped(char par1, int par2) {
        ...
        if ((par2 == 1)
                || (par2 == mc.gameSettings.keyBindInventory.getKeyCode())
