...
public class OresSearch {
    ...
    private static boolean checkList(
            List<OreInfo> temp,
            OreInfo value,
            ItemStack stack) // Used to check if a OreInfo already exists in the searchList
            {
        for (OreInfo oreCheck : temp) {
            if ((oreCheck.oreName == value.oreName)
                    && (oreCheck.id == Item.getIdFromItem(stack.getItem()))
    ...
    public static void add(
            String oreIdent, String name, int color) // Takes a string of id:meta or oreName to add to our search list.
            {
        ...
        if (oreIdent.contains(":")) // Hopefully a proper id:meta string.
        {
            ...
            try {
                ...
                String notify = String.format(
                        "[�aFgt XRay�r] %s contains data other than numbers and the colon. Failed to add.", oreIdent);
                ChatComponentText chat = new ChatComponentText(notify);
                mc.ingameGUI.getChatGUI().printChatMessage(chat);
                return;
            }
            ...
            try {
                ...
                String notify = String.format(
                        "[�aFgt XRay�r] Doesn't support in-game additions to the ore dictionary yet.. Failed to add.");
                ChatComponentText chat = new ChatComponentText(notify);
                mc.ingameGUI.getChatGUI().printChatMessage(chat);
                return;
            }
    ...
    public static List<OreInfo> get() // Return the searchList, create it if needed.
        ...
        if (OresSearch.searchList.isEmpty()) {
            ...
            Map<String, OreInfo> tempOredict =
                    new HashMap<String, OreInfo>(); // Temporary oredict map to replace oredictOres
            ...
            for (Map.Entry<String, OreInfo> entry : FgtXRay.oredictOres.entrySet()) {
                ...
                ArrayList<ItemStack> oreDictOres =
                        OreDictionary.getOres(key); // Get an itemstack array of all the oredict ores for 'key'
                if (oreDictOres.size() < 1) {
                    System.out.println(String.format(
                            "[Fgt XRay] Ore %s doesn't exist! Skipping. (We shouldn't have this issue here! Please tell me about this!)",
                            key));
                    continue;
                }
                for (int i = 0; i < oreDictOres.size(); i++) {
                    ...
                    temp.add(new OreInfo(
                            ...
                            value.draw));
                    System.out.println(String.format(
                            "[Fgt XRay] Adding OreInfo( %s, %d, %d, 0x%x, %b ) ",
                            value.oreName,
                            Item.getIdFromItem(oreItem.getItem()),
                            oreItem.getItemDamage(),
                            value.color,
            ...
            for (OreInfo ore : FgtXRay.customOres) // TODO: Check if custom already exists
            {
                System.out.println(String.format(
                        "[Fgt XRay] Adding OreInfo( %s, %d, %d, 0x%x, %b ) ",
                        ore.oreName, ore.id, ore.meta, ore.color, ore.draw));
                temp.add(ore);
            }
